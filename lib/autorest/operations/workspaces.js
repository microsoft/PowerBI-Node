 ...
 *                      See {@link ODataResponseListWorkspace} for more
 *                      information.
 *
 ...
 */
Workspaces.prototype.getWorkspacesByCollectionName = function (collectionName, options, callback) {
    ...
    if (statusCode === 200) {
      ...
      try {
        ...
        if (parsedResponse !== null && parsedResponse !== undefined) {
          var resultMapper = new client.models['ODataResponseListWorkspace']().mapper();
 ...
 *                      {stream} [response] - The HTTP Response stream if an error did not occur.
 */
Workspaces.prototype.postWorkspace = function (collectionName, options, callback) {
  var client = this.client;
  if(!callback && typeof options === 'function') {
    callback = options;
    options = null;
  ...
  var workspaceRequest = (options && options.workspaceRequest !== undefined) ? options.workspaceRequest : undefined;
  // Validate
  try {
    if (collectionName === null || collectionName === undefined || typeof collectionName.valueOf() !== 'string') {
      throw new Error('collectionName cannot be null or undefined and it must be of type string.');
  ...
  try {
    if (workspaceRequest !== null && workspaceRequest !== undefined) {
      var requestModelMapper = new client.models['CreateWorkspaceRequest']().mapper();
      requestModel = client.serialize(requestModelMapper, workspaceRequest, 'workspaceRequest');
      requestContent = JSON.stringify(requestModel);
    }
  } catch (error) {
    var serializationError = new Error(util.format('Error "%s" occurred in serializing the ' + 
        'payload - "%s"', error.message, util.inspect(workspaceRequest, {depth: null})));
  ...
  return client.pipeline(httpRequest, function (err, response, responseBody) {
    ...
    if (statusCode === 201) {
      ...
      try {
        ...
        result = JSON.parse(responseBody);
        if (parsedResponse !== null && parsedResponse !== undefined) {
          var resultMapper = new client.models['Workspace']().mapper();
          result = client.deserialize(resultMapper, parsedResponse, 'result');
        }
      } catch (error) {
        var deserializationError = new Error(util.format('Error "%s" occurred in deserializing the responseBody - "%s"', error, responseBody));
