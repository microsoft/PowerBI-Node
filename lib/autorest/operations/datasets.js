 ...
 *                      {stream} [response] - The HTTP Response stream if an error did not occur.
 */
Datasets.prototype.bindToGateway = function (collectionName, datasetKey, bindToGatewayRequest, options, callback) {
  var client = this.client;
  if(!callback && typeof options === 'function') {
    callback = options;
    options = null;
  ...
  // Construct URL
  var requestUrl = this.client.baseUri +
                   '//beta/collections/{collectionName}/datasets/{datasetKey}/Default.BindToGateway';
  requestUrl = requestUrl.replace('{collectionName}', encodeURIComponent(collectionName));
  requestUrl = requestUrl.replace('{datasetKey}', encodeURIComponent(datasetKey));
  // trim all duplicate forward slashes in the url
  var regex = /([^:]\/)\/+/gi;
  ...
  try {
    if (bindToGatewayRequest !== null && bindToGatewayRequest !== undefined) {
      var requestModelMapper = new client.models['BindToGatewayRequest']().mapper();
      requestModel = client.serialize(requestModelMapper, bindToGatewayRequest, 'bindToGatewayRequest');
      requestContent = JSON.stringify(requestModel);
    }
  } catch (error) {
    var serializationError = new Error(util.format('Error "%s" occurred in serializing the ' + 
        'payload - "%s"', error.message, util.inspect(bindToGatewayRequest, {depth: null})));
    return callback(serializationError);
  }
  httpRequest.body = requestContent;
  // Send Request
